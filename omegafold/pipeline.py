...
def csv_to_fasta(
    csv_path: str, fasta_path: str, aa_col_name: str, chain_ids_col_name: str = None
) -> None:
    df = pd.read_csv(csv_path)

    with open(fasta_path, "w") as f:
        aa_col: pd.Series = df[aa_col_name]
        if chain_ids_col_name is not None:
            chain_ids_col: pd.Series = df[chain_ids_col_name]
            for c, a in zip(chain_ids_col.values, aa_col.values):
                f.write(f">{c}\n")
                f.write(a + "\n")
        else:
            for a in aa_col.values:
                f.write(">\n")
                f.write(a + "\n")


def parse_fasta_file(fasta_path: str) -> typing.List:
    chain_ids: typing.List[str] = []
    aastr: typing.List[str] = []
...
def fasta2inputs(
    ...
    """
    combined = parse_fasta_file(fasta_path=fasta_path)
...
def _load_weights(weights_url: str, weights_file: str,) -> collections.OrderedDict:
    """
...
def get_args() -> typing.Tuple[
    ...
    forward_config = argparse.Namespace(
        subbatch_size=args.subbatch_size, num_recycle=args.num_cycle,
    )
